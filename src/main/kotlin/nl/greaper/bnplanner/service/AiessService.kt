...
class AiessService(
    ...
    fun processAiessBeatmapGamemodeEvent(
        ...
        val updatedBeatmap =
            beatmapService.updateBeatmapGamemodes(databaseBeatmap, changingGamemodes, status) { updatingGamemode ->
    ...
    fun createFallbackBeatmap(
        event: AiessBeatmapEvent,
        userId: String?,
        username: String?,
    ): Beatmap {
    ...
    fun processAiessBeatmapEvent(event: AiessBeatmapEvent): AiessResponse {
        if (instantStatus.none { it == event.status }) {
            if (event.userId != null && event.username != null) {
                val databaseBeatmap =
                    beatmapService.findBeatmap(event.beatmapSetId)
                ...
                if (event.status == BeatmapStatus.Disqualified || event.status == BeatmapStatus.Reset) {
                    val updatedGamemodes =
                        databaseBeatmap.gamemodes.map { gamemode ->
        ...
        val newNominators =
            when {
                ...
                currentFirstNominator.nominatorId == MISSING_USER_ID -> {
                    ...
                    beatmapService.logAiessUpdatedNominator(
                        databaseBeatmap,
                        currentFirstNominator.nominatorId,
                        userId,
                        updatingGamemode.gamemode,
                    )
                    logNomination(databaseBeatmap, beatmapStatus, userId, username, updatingGamemode.gamemode)
                    newNominator to currentSecondNominator
                }
                currentSecondNominator.nominatorId == MISSING_USER_ID -> {
                    ...
                    beatmapService.logAiessUpdatedNominator(
                        databaseBeatmap,
                        currentSecondNominator.nominatorId,
                        userId,
                        updatingGamemode.gamemode,
                    )
                    logNomination(databaseBeatmap, beatmapStatus, userId, username, updatingGamemode.gamemode)
    ...
    private fun logBeatmapAdded(
        ...
        val footer =
            if (nominatorId != null && username != null) {
                ...
                if (MeGroup.SupportedGroups.contains(event.groupId)) {
                    ...
                    val updateUser =
                        if (oldGamemode != null) {
                            ...
                            changingUser
                                .copy(gamemodes = untouchedGamemodes + updatedGamemode)
                            ...
                            changingUser
                                .copy(gamemodes = changingUser.gamemodes + newGamemode)
    ...
    fun logUserMove(
        user: User,
        event: AiessUserEvent,
        oldGamemode: UserGamemode?,
        updatedGamemode: UserGamemode?,
    ) {
        val color =
            if (event.type == AiessUserEventType.add) {
                EmbedColor.GREEN
            } else {
                EmbedColor.RED
            }
        ...
        val topLine =
            if (oldGamemode != null && updatedGamemode != null) {
